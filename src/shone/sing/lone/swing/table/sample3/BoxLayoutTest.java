...
public class BoxLayoutTest {
	...
	static void createMiddlePanel() {
		...
		sourceLabel.setPreferredSize(new Dimension(400, 400));
		sourceLabel.setAlignmentY(Component.TOP_ALIGNMENT);
		sourceLabel.setBorder(BorderFactory.createLineBorder(Color.red));
		// 创建列表运动会项目
		...
		sourceListPanel.setBorder(BorderFactory.createEmptyBorder(0, 30, 0, 30));
		...
		// 创建标签查询项目
		JLabel targetLabel2 = new JLabel("查询项目：");
		targetLabel2.setBackground(Color.green);
		targetLabel2.setAlignmentY(Component.TOP_ALIGNMENT);
		targetLabel2.setBorder(BorderFactory.createEmptyBorder(4, 5, 0, 5));

		// 创建列表查询项目
		DefaultListModel targetList2Model = new DefaultListModel();
		targetList2Model.addElement("100 米");
		JList targetList2 = new JList(targetList2Model);
		targetList2.setSelectionMode(ListSelectionModel.MULTIPLE_INTERVAL_SELECTION);
		targetList2.setVisibleRowCount(5);
		JScrollPane targetList2Scroller = new JScrollPane(targetList2);
		targetList2Scroller.setPreferredSize(new Dimension(120, 80));
		targetList2Scroller.setVerticalScrollBarPolicy(JScrollPane.VERTICAL_SCROLLBAR_ALWAYS);
		targetList2Scroller.setAlignmentY(Component.TOP_ALIGNMENT);
		// 创建最右边的 Panel
		JPanel targetList2Panel = new JPanel();
		// 设置最右边的 Panel 为水平布局
		targetList2Panel.setLayout(new BoxLayout(targetList2Panel, BoxLayout.X_AXIS));
		// 将标签查询项目加到最右边的 Panel
		targetList2Panel.add(targetLabel2);
		// 将列表查询项目加到最右边的 Panel
		targetList2Panel.add(targetList2Scroller);
		targetList2Panel.setAlignmentY(Component.TOP_ALIGNMENT);
		targetList2Panel.setBorder(BorderFactory.createEmptyBorder(0, 30, 0, 30));
		// 最后将最右边的 Panel 加入到 middlePanel
		middlePanel.add(targetList2Panel);
	}
